...
describe("External - SecurityAdapter", () => {
    ...
    test("Should return false | verifyBasicAuthenticationCredential", async () => {
        ...
        const credential = Buffer.from("error:error").toString("base64");
        ...
        expect(sut.verifyBasicAuthenticationCredential).toHaveBeenCalledWith(
            credential,
        );
    });
    ...
    test("Should return true | verifyBasicAuthenticationCredential", async () => {
        ...
        const credential = Buffer.from(
            `${environmentVariables.basicAuthenticationUser}:${environmentVariables.basicAuthenticationPassword}`,
        ).toString("base64");
        jest.spyOn(sut, "verifyBasicAuthenticationCredential");
