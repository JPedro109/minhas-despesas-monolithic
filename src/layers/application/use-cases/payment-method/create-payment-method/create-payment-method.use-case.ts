...
export class CreatePaymentMethodUseCase implements ICreatePaymentMethodUseCase {
	...
	async execute({ userId, name, token }: CreatePaymentMethodDTO): Promise<string> {
		...
		await this.unitOfWorkRepository.transaction(async () => {
			paymentMethodCreated = await paymentMethodRepository.createPaymentMethod(paymentMethod);
