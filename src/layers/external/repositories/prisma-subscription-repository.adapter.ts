...
export class PrismaSubscriptionRepositoryAdapter implements ISubscriptionRepository {
    ...
    async createSubscription(subscription: SubscriptionEntity): Promise<SubscriptionEntity> {
        const createdSubscription = await this.context.prismaSubscription.create({
            data: {
                id: subscription.id,
        ...
        return PrismaMapperHelper.toSubscriptionEntity(createdSubscription, createdSubscription.plan, createdSubscription.plan.actions);
    }
    ...
    async getActiveSubscriptionByUserId(userId: string): Promise<SubscriptionEntity | null> {
        ...
        return PrismaMapperHelper.toSubscriptionEntity(subscription, subscription.plan, subscription.plan.actions);
    }
    ...
    async getActiveSubscriptionsByEndDate(endDate: Date, renewable: boolean): Promise<SubscriptionEntity[]> {
        ...
        return subscriptions.map(
            subscription => PrismaMapperHelper.toSubscriptionEntity(subscription, subscription.plan, subscription.plan.actions)
        );
    }
