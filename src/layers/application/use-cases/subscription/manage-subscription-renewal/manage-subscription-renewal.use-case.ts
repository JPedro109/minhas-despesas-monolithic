...
export class ManageSubscriptionRenewalUseCase implements IManageSubscriptionRenewalUseCase {
    ...
    async execute({ userId, renew }: ManageSubscriptionRenewalDTO): Promise<void> {
        ...
        if (!subscriptionActive) throw new NotFoundError("Esse usuário não possui uma assinatura ativa");
        ...
        const newSubscription = renew
            ...
            : await this.handleNonRenewal(subscriptionActive, startOfToday, planRepository);
        ...
        if (newSubscription) {
            await this.unitOfWorkRepository.transaction(async () => {
                ...
                await subscriptionRepository.createSubscription(newSubscription);
                
                if (subscriptionActive.renewable) await expenseRepository.deleteExpensesByUserId(userId);
            });
        }
    }

    private createRenewedSubscription(subscriptionActive: SubscriptionEntity, startOfToday: Date): SubscriptionEntity {
        return new SubscriptionEntity({
            ...
            endDate: new Date(
                ...
                startOfToday.getUTCDate() + subscriptionActive.plan.durationInDays,
                0,
                0,
                0
            )
    ...
    private async handleNonRenewal(
        ...
        const daysLateInPayment = (startOfToday.getTime() - subscriptionActive.endDate.getTime()) / 1000 / 60 / 60 / 24;
