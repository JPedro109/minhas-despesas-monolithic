...
export class SendNotificationOfExpensesThatAreComingDueUseCase
    implements ISendNotificationOfExpensesThatAreComingDueUseCase
{
    constructor(
    ...
    async execute(): Promise<void> {
        ...
        const dueDateOfExpenses = new Date(
        ...
        const expenses =
            await expenseRepository.getExpensesByDueDate(dueDateOfExpenses);
        ...
        expenses.forEach((expense) => {
            ...
            if (!userExpenses) {
                expensesMap.set(expense.userId, [
                    {
        ...
        const userIds = Object.keys(Object.fromEntries(expensesMap));
        const users = await userRepository.getUsersByIds(userIds);

        const emails = [];
        users.forEach((user) => {
            const expenses = expensesMap.get(user.id);
            const email = this.notification.sendMail(
                user.email,
