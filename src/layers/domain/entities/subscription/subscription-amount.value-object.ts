...
export class SubscriptionAmountValueObject {
    ...
    static create(subscriptionAmount: number): SubscriptionAmountValueObject | InvalidSubscriptionAmountError {
        if (!this.validate(subscriptionAmount)) return new InvalidSubscriptionAmountError();
