...
export class SetFreePlanForNonRenewableSubscriptionsController extends AbstractController {
    ...
    protected async handler(): Promise<HttpResponse> {
        const subscriptions = await this.getActiveNonRenewableSubscriptionsUseCase.execute();

        const promisses = [];
        for(const subscription of subscriptions) {
            promisses.push(this.manageSubscriptionRenewalUseCase.execute({ userId: subscription.userId, renew: false }));
        }
