...
export class SecurityAdapter implements ISecurity {
    ...
    createJsonWebToken(payload: object, expiryTimeInSeconds: number): string {
        return this.jsonWebToken.sign(payload, environmentVariables.jwtKey, {
    ...
    verifyJsonWebToken(token: string): JsonWebTokenType {
        try {
            return this.jsonWebToken.verify(
                token,
                environmentVariables.jwtKey,
            ) as JsonWebTokenType;
        } catch (e) {
    ...
    verifyBasicAuthenticationCredential(credential: string): boolean {
        ...
        return (
            username === environmentVariables.basicAuthenticationUser &&
            password === environmentVariables.basicAuthenticationPassword
        );
    }
}
